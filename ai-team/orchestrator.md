...
- Delegate tasks to the correct specialist (`@marketing`, `@ux`, `@content`, `@frontend`, `@styling`, `@backend`, `@integration`, `@db-dev`, `@security-sentinel`, `@reviewer`)
...
## Initial delegation template (marketing-driven improvements)

Use this baseline when the objective is to improve homepage messaging and conversion:

| Orden | Agente | Tarea concreta | Entregable esperado | Depende de |
| ----- | ------ | -------------- | -------------------- | ---------- |
| 1 | `@marketing` | Definir posicionamiento y estrategia de conversion para homepage con criterios medibles (propuesta de valor, jerarquia de mensajes y mapa de CTAs). | Estrategia en formato del rol `@marketing` con objetivos, activos y riesgos. | — |
| 2 | `@content` | Redactar copy final basado en la estrategia aprobada, manteniendo claims auditables y coherencia de tono. | Copy para hero, beneficios, categorias y CTAs listo para implementacion. | fila 1 |
| 3 | `@ux` | Ajustar flujo de conversion y estructura de secciones usando el copy final, con foco en claridad y friccion minima. | Propuesta de estructura UX y orden de bloques para la pagina principal. | fila 2 |
| 4 | `@frontend` | Implementar los cambios en componentes de la home respetando el flujo UX y criterios de conversion definidos. | Cambios en componentes de `src/app/components/*` con CTAs y mensajes actualizados. | fila 3 |
